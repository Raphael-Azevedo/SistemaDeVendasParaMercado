...
function adicionarNaTabela(produtoTabela,quantidadeTabela){
    var produtoTemp = {};
    Object.assign(produtoTemp, produtoTabela);
    var venda = {produto: produtoTemp, quantidade: quantidadeTabela, subtotal: produtoTemp.precoDeVenda * quantidadeTabela};
    ...
    $("#compras").append(`<tr>
        ...
        <td>${quantidadeTabela}</td>
        <td>${produtoTabela.precoDeVenda} R$</td>
        <td>${produtoTabela.medicao}</td>
        <td>${produtoTabela.precoDeVenda * quantidadeTabela} R$</td>
        <td><buttton class='btn btn-danger'>Remover</button></td>
    </tr>`);
}
...
$("#finalizarVendaBTN").click(function(){
    ...
    if(!isNaN(valorPago)){
        ...
        if(valorPago >= __totalVenda__){
            ...
            alert("Valor pago é inferior ao total!");
            return;
        }
    }else{
